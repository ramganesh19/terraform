...
resource "bigip_ltm_policy" "waf-policy" {
  name      = "/Common/asm-terraform-l7-waf-policy"
...
resource "bigip_ltm_virtual_server" "https" {
        ...
        profiles = [ "/Common/tcp", "/Common/http", "/Common/websecurity" ]
        client_profiles            = ["/Common/clientssl"]
        #server_profiles            = ["/Common/serverssl"]
        policies =[bigip_ltm_policy.waf-policy.name]
        depends_on = [bigip_ltm_pool.pool, bigip_ltm_policy.waf-policy]
}
